...
. Wenn der Zielordner sich auf einem Netzlaufwerk befindet, kann aus Sicherheitsgründen Git das eben geclonte Repository nicht nutzen (Einstellung: https://git-scm.com/docs/git-config/2.35.2#Documentation/git-config.txt-safedirectory[safe.directory]). Folgendes ist noch zu tun:
+
[IMPORTANT]
====
. Terminal öffnen: "View > Terminal"
