...
+
Sie haben die Wahl, ob Sie einen neuen Account für ihre HTWD E-Mail-Adresse erstellen oder sie ihrem bestehenden Account ihre HTWD E-Mail-Adresse über "Settings > Emails" hinzufügen.

TIP: Ist dem GitHub-Account die HTWD E-Mail-Adresse hinzugefügt oder dieser direkt damit angelegt worden, können über das https://education.github.com/students[GitHub Education] Programm weitere Features von GitHub freigeschaltet und an weiteren Bonusprogrammen teilgenommen werden.
...
[NOTE]
======
Seit dem 13. August 2021 akzeptiert GitHub für die Authentifizierung von Git-Operationen über HTTPS nur noch *GitHub OAuth* oder *Personal access tokens* statt des Account Passwortes. (https://github.blog/2020-12-15-token-authentication-requirements-for-git-operations/[Quelle])
...
Die angegebene E-Mail dient zur Identifizierung ihrer Änderungen am Repository und zur Zuordnung ihres GitHub Accounts. Der Proxy ist notwendig, damit Sie aus dem geschützten HTWD-Netz (Labore, VPN, WLAN) sich mit dem Internet verbinden können.
...
. Öffnen Sie in Visual Studio Code über "File > Open..." die schon vorhandene Datei _.gitconfig_ oder legen Sie eine neue Datei über "File > New File" an. Passen Sie den Inhalt wie folgt an:
+
...
----
+
Für die E-Mail verwenden Sie bitte eine ihrer HTWD Adressen mit `s00000@informatik.htw-dresden.de`, `s00000@htw-dresden.de` oder `vorname.nachname@htw-dresden.de`, welche in Ihrem GitHub-Account verwendet wird.
...
. Zum Testen der Konfiguration öffnen Sie in Visual Studio Code über "View > Terminal"  bzw. kbd:[Ctrl+ö] oder kbd:[Ctrl+Shift+´] das integrierte Terminal oder ein beliebiges anderes Terminal (Windows: PowerShell) und geben folgende Git-Kommandos ein:
+
...
. Es erfolgt die Abfrage der Authentifizierung mit GitHub. Hier gibt es jetzt je nach Installation verschiedene Varianten. Entweder wird der Login per *GitHub OAuth* und der *Access Token* abgefragt oder Visual Studio Code nutzt seine *Access to GitHub* Erweiterung.
+
Verwenden Sie: *GitHub OAuth* (oder den in <<aufgabe_access_token,Aufgabe 1.4>> zuvor angelegten *Access Token*)
...
. Übernehmen Sie in die Datei _Beispieldokumentation.adoc_ das folgende AsciiDoc-Beispiel und passen Sie den Autor an Ihren Namen und das Datum an das heutige an.
+
Infos zur Syntax: https://asciidoctor.org/docs/asciidoc-syntax-quick-reference/[AsciiDoc Syntax Quick reference] und https://asciidoctor.org/docs/user-manual/[Asciidoctor User Manual]
+
