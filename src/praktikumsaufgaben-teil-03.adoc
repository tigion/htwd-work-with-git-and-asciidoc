...
.icon:user[] Person A
Erstellen Sie in Ihrem GitHub-Account ein neues *_privates_* Repository *htw-se-example-teamproject* mit folgenden Einstellungen:

  * Repository name: `htw-se-example-teamproject`
...
.icon:user[] Person A
Fügen Sie Ihrem Repository *htw-se-example-teamproject* über die Einstellungen "Settings > Manage access > Collaborators > Invite a collaborator" ihre Teammitglieder über den Account-Namen oder die E-Mail hinzu.
...
Schauen Sie in Ihr Mail-Postfach und nehmen Sie die Einladung zum *htw-se-example-teamproject* an. Öffnen Sie anschließend das Team-Repository auf GitHub von Person A.
...
.icon:users[] Person A, B, C
Clonen Sie Ihr gemeinsames Repository *htw-se-example-teamproject* von Person A mit Ihrem jeweiligen GitHub Account an einen geeigneten Platz als lokalen Workspace auf ihren Rechner.
...
====
=====
Beim Senden (*push*) der Änderungen des lokalen Repositories zum Remote-Repository kann es zu einem Konflikt bzw. Fehler kommen, wenn das Remote-Repository in der Zwischenzeit neuere Versionen (_commits_) erhalten hat.
...
====
=====
Sie haben in Ihrem lokalem Repository unversionierte oder noch im INDEX vorgemerkte Änderungen, welche auf einer älteren Version als das Remote-Repository basieren.

Variante 1 - lokale unversionierte Änderung erst in das Repository aufnehmen::
. Nehmen Sie Ihre lokale unversionierte Änderung (_add_ und _commit_) in Ihr Repository auf.
...
Variante 2 - lokale unversionierte Änderung verwerfen::
...
Variante 3 - lokale unversionierte und vorgemerkte Änderung verwerfen::
. Mit `git reset --hard` werden alle Änderungen im INDEX und WORKSPACE gegenüber Ihrer letzten lokalen Version verworfen.
. Anschließend können Sie mit *pull* die aktuelle Version des Remote-Repositories laden.

Weitere Varianten::
- Mit https://git-scm.com/docs/git-stash[stash] können Sie lokale unversionierte Änderungen temporär Zwischenspeichern und Ihren lokalen Stand auf die letzte Version Ihres lokalen Repositories zurücksetzen.
=====
====
...
.icon:user[] Person A
1. Erstellen Sie im lokalen Workspaces Ihres *htw-se-example-teamproject* Repositories ein Verzeichnis *docs* und legen Sie in diesem eine neue Datei *_Teamdokumentation.adoc_* mit folgendem Inhalt in Visual Studio Code an:
+
...
Bearbeiten Sie im Team die folgenden Aufgaben und nehmen Sie die gemachten Änderugen jeweils in Ihr lokales und Ihr entferntes Teamrepository auf GitHub auf. Es ist keine bestimmt Reihenfolge vorgegeben, nur was wer tun soll. Diskutieren Sie Ihr Vorgehen und auftretende Problemen. Probieren Sie etwas und versuchen Sie aufgetretene _merge_-Konflikte zu lösen.

NOTE: Mehrere Autoren werden in eine Zeile getrennt mit `;` geschrieben. Besteht ein Autorenname aus mehr als drei Teilen, müssen die Vor- und Nachnamen mit `_` jeweils zu einer Gruppe zusammengefasst werden.
...
- Ergänzen Sie in der Datei *_Teamdokumentation.adoc_* im Dokumentenkopf Ihren Namen und Ihre E-Mail.
- Generieren Sie mit `asciidoctor` das finale HTML-Document.
...
Eine weitere Möglichkeiten für die Zusammenarbeit über GitHub sind *Pull requests*, welche wir uns im nächsten Semester (<<praktikumsaufgaben-teil-08.adoc#, Teil 8 - Zusammenarbeit 3>>) anschauen.
