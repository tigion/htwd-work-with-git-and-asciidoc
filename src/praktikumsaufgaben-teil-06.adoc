...
====
Nutzen Sie die Möglichkeiten sich auf Webseiten wie der offiziellen http://git-scm.com/documentation[Git Dokumentation] oder anderen Resourcen über die verwendeten git-Kommandos und -Konzepte zu informieren.
...
- *Änderung* ... unversionierte Änderungen im lokalen Arbeitsverzeichnis
- *Version* ... versionierte Änderungen (Commits) im lokalen Arbeitsverzeichnis
...
<3> stellt die weggespeicherte Änderung wieder her und entfernt den zugehörigen _Stash_
<4> stellt die weggespeicherte Änderung wieder her und behält den zugehörigen _Stash_ (nützlich, wenn die Änderung noch an anderer Stelle (bspw. anderer _Branch_) gebraucht wird)
<5> entfernt den Stash mit den enthaltenen Änderungen
<6> Liste der vorhandenen _Stashes_
<7> ... zeigt Kurzform der Änderungen im _Stash_ gegenüber der aktuellen Version
<8> ... zeigt auch inhaltliche Änderungen (Patch-Form)
...
$ git clean -df <4>
----
<1> Setzt lokale Änderungen einer oder mehrerer versionierter Dateien auf die letzte Version zurück. Ein `.` statt `<file(s)>` nimmt alle lokalen Änderungen.
...
<4> Entfernt alle lokalen unversionierte Dateien. (https://git-scm.com/docs/git-clean[git-clean Dokumentation], `-n` Testlauf, `-i` interaktiv, `-d` beachte auch unversionierte Verzeichnisse, `-f` ignoriere Löschwarnung)
=====
====
