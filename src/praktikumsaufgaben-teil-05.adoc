...
====
Hinweise zur Installation von *asciidoctor-diagram* und *Graphviz* und der Einrichtung von *Visual Studio Code* für icon:windows[] Windows, icon:linux[] Linux und icon:apple[] macOS finden Sie unter https://www.informatik.htw-dresden.de/~zirkelba/notes/se/asciidoc-installation-and-configuration/[Asciidoctor - Installation und Konfiguration].
...
+
* Asciidoc: *Use_asciidoctor_js*: icon:check-square-o[] (aktiviert)
* Asciidoc: *Use_kroki*: icon:check-square-o[] (aktiviert)
...
<2> Attribute für die Generierung mit der Asciidoktor-Diagramm Erweiterung
...
Hilfreich zum Layouten bzw. zur Positionierung von Elementen ist auch ein Blick in den entsprechenden Guide zu https://crashedmind.github.io/PlantUMLHitchhikersGuide/layout/layout.html[The Hitchhiker’s Guide to PlantUML - Layout].

.Hinweise und Tipps zur Anwendung der PlantUML Notation
[%collapsible]
====
=====
* In der Regel definiert man erste die Elemente (Type, Bezeichnung) und nachfolgend die Beziehungen zwischen ihnen. Elemente in Kontainern/Paketen, werden am Anfang des entspechenden Kontainers definiert.
...
* Wird das Attribut `subs="attributes+"` bei der Einbindung in Asciidoctor verwendet, können innerhalb des PlantUML-Diagrammes Variablen vom Asciidoctor-Dokument verwendet werden.
+
----
:variable1: Human
:variable2: Hallo
...
usecase "Anweundgsfall 1" as uc01
