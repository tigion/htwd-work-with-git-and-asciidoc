...
. Erstellen Sie in der *Overview* Sicht vier *Tasks* als neue Issues mit dem Label `task`, wovon sich die *zwei ersten* auf die vorhandene User Story beziehen sollen (als Sub-Issue zur User Story oder als Referenz `#<number>`). Legen Sie sich jeweils als zugeordneter Nutzer fest. Schauen Sie sich Ihre *User Story* (Issue) an, ob die Referenzen aufgetaucht sind. Bspw.:
+
.Tasks
[cols="1,4,8"]
|===
