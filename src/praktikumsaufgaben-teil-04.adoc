...
.#icon:warning[] new, todo icon:warning[]# Hinweise zu Projekten (Projects) in GitHub
[%collapsible]
====
=====
NOTE: Siehe auch https://docs.github.com/en/issues/planning-and-tracking-with-projects/learning-about-projects[]
...
** Mit einem Klick auf den Titel *@tigion's untitled project* wird der Namen des Projektes in bspw. *Iteration 1* geändert.

* Neue Aufgaben (Issues) können wie folgt hinzugefügt werden:
** Spalte *Todo* -> kbd:[+ Add item]:
*** per Suche -> Repository Namen eingeben und auswählen -> Issue direkt wählen oder per *Add multipleItems* gleich mehrere aus demgewählten Repository auswählen
*** per kbd:[+] -> *Add item from repository* -> Repository wählen -> *Issues* auswählen -> kbd:[Add selected items]

* Weitere Sichten (Views) für das Projekt anlegen:
** über den Button kbd:[+ New view] können weitere alternative Sichten als *Table* oder *Bord* hinzugefügt werden.

:imageTitle: GitHub Projekt - Board view
:imageFile: screenshot_github_project_board_view.png
.{imageTitle}
image::{imageFile}["{imageTitle}", width=800, link="{imagesdir}/{imageFile}"]

:imageTitle: GitHub Projekt - Table view - gruppiert per Status
:imageFile: screenshot_github_project_table_view.png
.{imageTitle}
image::{imageFile}["{imageTitle}", width=800, link="{imagesdir}/{imageFile}"]
...
[WARNING]
======
Projekte sind losgelöst vom Repository, deswegen müssen Teammitglieder extra dafür eingeladen werden.
======

* Teammitglieder zu Projekt einladen:
...
. Erstellen Sie eine *Iteration*, bspw.: `Iteration 01`, als Project mit dem Template: *Board* und nehmen Sie Ihre drei Work Items (Issues) auf.
+
...
. Schließen Sie ein *Work Item* (Issue) per Commit oder in der Issue-Anicht und schauen Sie sich die Änderung im *Iterations*-Projekt an.
