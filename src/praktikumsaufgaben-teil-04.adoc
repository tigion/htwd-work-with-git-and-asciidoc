...
- Issues können in Projekten zwischen den Zuständen mit Drag&Drop oder durch bestimmte Schlüsselwörter (automatisiert) bewegt werden.
...
.#icon:warning[] new, todo icon:warning[]# Hinweise zu Projekten (Projects) und Planung von Iterationen in GitHub
[%collapsible]
====
=====
...
* Erfassung einer Complexity/Velocity zu den Work Items (Issue) in den Iterationen
...
. Legen Sie in Ihrem privaten pository eine *User Story* als neue Issue mit dem Label `user story` an. Bspw.:
...
. Schließen Sie ein *Work Item* (Issue) per Commit oder in der Issue-Anicht und schauen Sie sich die Änderung im Projekt bzw. den Sichten an.
