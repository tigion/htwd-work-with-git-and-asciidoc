...
Ziel: Kennenlernen der Automatisierung von Softwareentwicklungsprozesse mit Hilfe von *Workflows* und *Action* auf *GitHub*
...
<13> ... `run: |` Führt ein *mehrzeiliges* Shell-Kommand aus.



.Beispiel eines komplexeren Events
[#example_event]
Folgend ein Beispiel für ein Event, welches auf *Push* und *Pull-Requests* für den Branch *main* und eine bestimmte Datei _data/colors.json_ reagiert:
...
Bearbeiten Sie die Aufgaben in Ihrem vorhandenem `htw-se-example-project`-Repository oder legen Sie sich dafür ein neues Repository auf GitHub an. Die Aufgabe orientiert sich am Einführungsbeispiel: https://docs.github.com/en/actions/quickstart[Quickstart for GitHub Actions] von GitHub.
...
Dafür verwenden wir das bereitgestellte https://github.com/tigion/htw-se-practical-github-actions-exercise[htw-se-practical-github-actions-exercise] Repository. Sie können dies gern mit Ihren Klassen aus dem Praktikum zum Thema *UnitTest mit Java und Python* erweitern bzw. anpassen.

. Erstellen Sie sich vom Repository https://github.com/tigion/htw-se-practical-github-actions-exercise[htw-se-practical-github-actions-exercise] einen Fork in Ihren GitHub Account und clonen ihn anschließend lokal auf Ihren Rechner.
...
@residents.setter
    def residents(self, value):
        ...
        # variant: exception
        #if value < 0:
        #    raise ValueError("residents must be greater than 0")
        #self.__residents = value
----
====
. Überlegen Sie sich eine Erweiterung des Workflows, welche Sie als weiteren Step hinzufügen. Dies könnte bspw. die Ausgabe der Java- oder Python-Version oder ein Test, ob eine Datei exisitiert sein. Testen Sie anschließend durch Änderungen am Projekt, ob Ihre Anpassungen im Workflow funktionieren.
...
. Schauen Sie sich im Demo-Projekt https://github.com/tigion/htw-playground-fork-colors[htw-playground-fork-colors] aus dem Praktikum <<praktikumsaufgaben-teil-08.adoc#_aufgabe_1_2_fork_mit_pull_request, Teil 6 - Aufgabe 1.2 Fork mit Pull-Request>> den enthaltenen Workflow an. Dieser enthält Beispiele für komplexere Shell Kommandos und Testmöglichkeiten.
